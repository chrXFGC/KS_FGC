...
if(@ARGV != 3){
	print "Usage: perl $0 <reference> <pileup> > <out>\n";
	exit;
}
...
open OUT, "| gzip > $out_file" or die $!;

print OUT "#Chr\tPos\tRef\tChain\tTotal\tMet\tUnMet\tMetRate\tRef_context\tType\n";
my %hash;
...
while(<IN>){
	...
	$rate = $dot/$total;
	if (defined $Ctype){
		print OUT "$chr\t$pos\t$ref\t$chain\t$total\t$dot\t$base\t$rate\t$str\t$Ctype\n";
	}
}
close IN;
close OUT;
